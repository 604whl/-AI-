...
import com.shortvideoscripagent.xhsagentyunying.auth.dto.LogoutRequest;
...
import com.shortvideoscripagent.xhsagentyunying.auth.dto.UpdateProfileRequest;
...
public class AuthService {
    ...
    private static final Set<String> VALID_PERSONAS = Set.of("agency", "mentor", "senior");
    ...
    public UserProfileVo updateProfile(Long userId, UpdateProfileRequest request) {
        ...
        if (request.getDefaultPersona() != null && !request.getDefaultPersona().isBlank()) {
            String persona = request.getDefaultPersona().trim();
            if (!VALID_PERSONAS.contains(persona)) {
                throw new BusinessException(CODE_BAD_REQUEST, "invalid_persona");
            }
    ...
    private UserProfileVo toProfile(User user) {
        return UserProfileVo.builder()
                ...
                .dailyQuota(user.getDailyQuota() == null ? 3 : user.getDailyQuota())
                .build();
    }
